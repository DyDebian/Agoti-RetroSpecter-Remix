...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		//Preload some things
		if(SONG.song.toLowerCase() == 'a.g.o.t.i-remix')
		{
			dad = new Character(100, 100, 'agoti-cut-1');
			add(dad);
			
			remove(dad);

			dad = new Character(100, 100, 'agoti-cut-2');
			add(dad);

			remove(dad);

			dad = new Character(100, 100, 'angry-agoti');
			add(dad);

			remove(dad);

			boyfriend = new Boyfriend(770, 450, 'bf-car');
			add(boyfriend);
		...
		//Preload NoteSplash
		var noteSplash:NoteSplash = new NoteSplash(0,0);
		noteSplash.animation.play('splash 0');
		add(noteSplash);
			...
			case 'void':
				{
					...
					void.frames = Paths.getSparrowAtlas('AgotiStage1/The_void');
					...
					bgRocks = new FlxSprite(-1000, -700).loadGraphic(Paths.image('AgotiStage1/Void_Back'));
					bgRocks.setGraphicSize(Std.int(bgRocks.width * 0.5));
					bgRocks.antialiasing = true;
					bgRocks.scrollFactor.set(0.7, 0.7);
					add(bgRocks);

					frontRocks = new FlxSprite(-1000, -600).loadGraphic(Paths.image('AgotiStage1/Void_Front'));
					...
					bgpillar.frames = Paths.getSparrowAtlas('AgotiStage1/Pillar_BG_Stage');
					...
					bgpillar.visible = false;
					add(bgpillar);

					speaker = new FlxSprite(-650, 600);
					speaker.frames = Paths.getSparrowAtlas('AgotiStage1/LoudSpeaker_Moving');
	...
	{
		remove(dad);
		dad = new Character(100, 100, 'agoti-no-mic');
		reloadAgotiPosition();
		add(dad);
		...
		new FlxTimer().start(1.9, function(tmr:FlxTimer)
		{
			...
			{
				dad.playAnim('idle');
				FlxG.sound.play(Paths.sound('introcut'));
				dad.animation.finishCallback = function(song)
				{
					remove(dad);
	...
	override public function update(elapsed:Float)
	{
		...
		if (executeModchart && luaModchart != null && songStarted)
		{
			...
			camHUD.x = luaModchart.getVar('hudX','float');
			camHUD.y = luaModchart.getVar('hudY','float');
	...
	private function popUpScore(daNote:Note):Void
		{
			...
			var noteSplash:NoteSplash = new NoteSplash(playerStrums.members[daNote.noteData].x,playerStrums.members[daNote.noteData].y);
			if(daRating == 'sick')
			{
				noteSplash.cameras = [camHUD];
				noteSplash.animation.play('splash ' + daNote.noteData);
				add(noteSplash);
			}
	...
	*/
	...
	function goCrazy()
	{
		...
		bgRocks.visible = false;
		frontRocks.visible = false;

		bgpillar.visible = true;
		speaker.visible = true;

		iconP2.animation.play('angry-agoti');
	}

	function reloadAgotiPosition()
	{
		switch(dad.curCharacter)
		{
			default:
